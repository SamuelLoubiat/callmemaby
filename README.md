...
call me maybe is a function calling tool that translates natural language prompts into structured, machine-executable function calls. Given a prompt like *"What is the sum of 21 and 42?"*, the system identifies the correct function and extracts the right arguments:

```json
{
  "name": "fn_add_numbers",
  "parameters": { "a": 21.0, "b": 42.0 }
}
```
...
- Python 3.10+
- uv package manager
- The llm_sdk package placed at the root of the project
...
This installs all dependencies declared in pyproject.toml into a virtual environment managed by uv.
...
The vocabulary JSON maps token IDs to their string representations. This mapping is used to pre-compute which tokens can legally appear at each position in the target schema.
...
- **No private SDK internals**: Only the public API of llm_sdk is used
---
...
| Function selection accuracy | ~90% on provided test cases                 |
...
Strings like fn_add_numbers often split into subwords. The constraint engine must validate tokens as valid continuations of a sequence, not just standalone items.

**2. Number tokenization**
Numbers (e.g., 265.5) tokenize differently based on context. Handling negatives, decimals, and leading zeros required strict constraint logic.

**3. Ambiguous prompts**
Some prompts fit multiple functions. While the LLM handles selection well, these edge cases required extra prompt engineering.

**4. Performance on CPU**
Running a 0.6B model token-by-token on CPU is slow. Batch pre-computing token masks and caching vocabulary lookups fixed the bottleneck.
...
- Qwen3 Model Card — HuggingFace— Documentation for the base model used
- Pydantic Documentation — Data validation library used throughout the project
- JSON Schema Specification — Reference for schema-aware generation

### AI Usage

Gemini was used to assist with:
